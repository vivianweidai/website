...
- 🌙 **Before planning an observing night, READ `work/astronomy/output/setup/night_run.py` FIRST — it usually already holds tonight's plan.** Its `PLAN` list and the comment block above it are the real plan of record: targets, exposures, the time-critical event, and why other targets were cut. It is edited *on the day of the run*, so it is current far more often than it looks. `plan_night.py` generates the coordinates (they go stale — minor planets move); `quick_look.py` is the mandatory pre-flight; `tree_check.py` measures horizon obstructions but **has never worked** (daylight exposure floor clamps to 10 s).
  **Do not design a session from scratch without checking it.** 2026-07-31: a full chunked schedule was proposed *and partly executed* — M13 imaging, drift tests, a target-selection question put to James — before anyone noticed that night's single-target SW Lac plan had been written that same afternoon, complete with the 01:46 primary-minimum timing and a launch-by-21:50 budget. The observing window is the scarce resource; rediscovering a plan that already exists spends it.
