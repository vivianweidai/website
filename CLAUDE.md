...
**Physics problem leads are generated.** Each `work/physics/problems/<n> <name>/PAPER Leads.pdf` (paywalled papers worth retrieving by hand) is built from **`work/physics/leads.yml`** by `python3 work/physics/build_leads.py` (optionally with problem numbers: `… build_leads.py 8 13`). Edit the YAML and rebuild; never hand-edit the PDFs. Rendering is Chrome headless `--print-to-pdf`, same as `work/overview.pdf`.
