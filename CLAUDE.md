...
|---|---|---|---|---|
| Topic group | `categories[].category` | `category:` | — | — |
| Tech (row) | `categories[].techs[].tech` | `tech:` | `/research/technology/<sci>/<Tech>/` | `tech` |
...
- **Images** — soft cap 500 KB / 1600 px wide. Resize with `sips -Z 1600 -s formatOptions 82 file --out file`. Convert PNGs without transparency to JPEG (`sips -s format jpeg`); for PNGs with transparency that must stay PNG, `pngquant --quality=65-85 --ext .png --force file.png`. The pre-commit hook in `.githooks/pre-commit` warns on staged offenders (does not block). Activate once: `git config core.hooksPath .githooks`.
  ...
  *Exception — tech hero images.* Files at `public/research/technology/<sci>/<Tech>/<hero>.{png,jpeg}` render at full retina width and must stay at source resolution. Keep PNG lossless or JPEG q90+. Do NOT shrink. The pre-commit hook auto-skips this path.
...
**These tables must stay in sync with `public/research/technology.yml`.** The category, tech name, and spec/description in each row are duplicates of `technology.yml` fields (`categories[].category`, `categories[].techs[].tech`, `categories[].techs[].specs`). When you edit a tech's spec or category in `technology.yml`, sweep every project's Technology table that lists that tech and update the duplicated text — otherwise the tech card and the project page disagree.
