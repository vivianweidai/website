...
| `.eq` | a display equation, with `<small>` for the line explaining it; see the math rule below for when to typeset it |
...
- Pitch at Vivian's level, not at the writer's. These pages are scaffolding for her, so what she already knows sets the floor. The Curriculum vertical next door is the first check: material sitting in `curriculum/content/<subject>/` is probably hers already, and re-teaching it puts padding between her and the part that is new. Hydrogen energy levels and the Balmer ladder came out of the first report for exactly that reason.
- Depth goes where no curriculum reaches. The Bayer mosaic, the point spread function, a saturated core wrecking a centroid, a streak beating against the pixel grid — none of it is taught anywhere, all of it is real, and that is what the length is for. Cutting basics and expanding these is the same rule applied twice, not a contradiction.
- But the curriculum check is a prompt, not a verdict, and it fails in two directions. A fresh angle on familiar material earns its place — new knowledge sitting on top of old is exactly how she iterates, and a known idea arriving from an unexpected direction is worth more than a novel one arriving cold. And coverage is not fluency: some subjects have been worked over far harder than others, so a topic can be nominally in the curriculum and still be thin. Noise adding in quadrature and the silicon bandgap both stayed in the first report on those grounds. When unsure, ask rather than cut.
- The temptation to resist through all of this is writing for whoever is enjoying the page most, which is usually the author.
...
- Typeset math only where the shape carries the meaning. An integral, a fraction, a sum with limits — anything that has to stack vertically to be read — goes in a `.eq` box as `$$ ... $$`, with a blank line above and below the math so markdown keeps parsing inside the div. Simple algebra stays on one line as plain text: `px(λ) = A · tan( asin( λ / 10,000 nm ) )` gains nothing from being typeset and is more compact vertically. The renderer is MathJax in SVG mode, wired up in `astro.config.mjs`, and the mode matters — it emits vector paths rather than positioning glyphs from a web font. A font-based renderer looks perfect until the font fails to load, at which point fraction bars cut through the letters and the page still returns 200, so nothing tells you it broke. KaTeX was tried first and did exactly that, because Vite inlines an imported stylesheet in dev and re-bases its relative font paths against the page URL.
